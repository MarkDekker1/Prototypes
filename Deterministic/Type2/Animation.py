...
def simData():
    ...
    while t < t_max:
        if not pause:
            ...
            d1y = potentialx(xvec[np.int(t/dt)],yvec[np.int(t/dt)],zvec[np.int(t/dt)])
            d2x = yvec[np.int(t/dt)]
            d2y = potentialy(xvec[np.int(t/dt)],yvec[np.int(t/dt)],zvec[np.int(t/dt)])
            d3x = zvec[np.int(t/dt)]
            d3y = potentialz(t,xvec[np.int(t/dt)],yvec[np.int(t/dt)],zvec[np.int(t/dt)])
